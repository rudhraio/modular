...
signIn.post("/", validator(validate), async (req, res) => {
    try {
        ...
        logger(`userBusiness ${JSON.stringify(userBusiness)}`);

        // get business data
        const business = await businessRespo.getOne({ where: { id: userBusiness.business } });
        logger(`business ${JSON.stringify(business)}`);



        // generate token
        const token = generateToken({ id: user?.id, domain: business.id, user_type: userBusiness.user_type });
        ...
        return successResponse(res, "Login successfull", { ...user, domain: business.id, user_type: userBusiness.user_type });
    } catch (err) {
        logger(`[FROM]: signin \n[ERR]: ${err}`);
